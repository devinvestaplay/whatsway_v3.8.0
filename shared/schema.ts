...
export const shopifyStores = pgTable(
  ...
  (table) => ({
    userIdIdx: index("shopify_stores_user_id_idx").on(table.userId),
    shopDomainIdx: index("shopify_stores_shop_domain_idx").on(
      table.shopDomain
    ),
  })
);
...
export type InsertShopifyStore =
  typeof shopifyStores.$inferInsert;

export type ShopifyStore =
  typeof shopifyStores.$inferSelect;


export const woocommerceStores = pgTable("woocommerce_stores", {
  id: varchar("id")
    .primaryKey()
    .default(sql`gen_random_uuid()`),
...
export type WooCommerceStore =
  typeof woocommerceStores.$inferSelect;

export type InsertWooCommerceStore =
  typeof woocommerceStores.$inferInsert;
...
export const codVerifications = pgTable("cod_verifications", {
  ...
  currency: varchar("currency", { length: 10 }),   // ✅ add if needed
...
export const abandonedCarts = pgTable(
  ...
  (table) => ({
    storeIdx: index(
      "abandoned_carts_store_idx"
    ).on(table.storeId),

    channelIdx: index(
      "abandoned_carts_channel_idx"
    ).on(table.channelId),

    contactIdx: index(
      "abandoned_carts_contact_idx"
    ).on(table.contactId),

    statusIdx: index(
      "abandoned_carts_status_idx"
    ).on(table.status),
  })
);
...
export type AbandonedCart =
  typeof abandonedCarts.$inferSelect;

export type InsertAbandonedCart =
  typeof abandonedCarts.$inferInsert;
...
//     storeId: varchar("store_id").references(
//   () => shopifyStores.id,
//   { onDelete: "set null" }
// ),
...
export const contacts = pgTable(
  "contacts",
  {
    ...
    // manual, import, api, chatbot, shopify
    source: varchar("source", {
      length: 100,
    }),

    // Shopify / WooCommerce / Magento Store
    storeId: varchar("store_id").references(
      () => shopifyStores.id,
      {
        onDelete: "set null",
    ...
    createdBy: varchar("created_by").references(
      (): any => users.id,
      {
        onDelete: "set null",
      }
  ...
  (table) => ({
    contactChannelIdx: index(
      "contacts_channel_idx"
    ).on(table.channelId),

    contactPhoneIdx: index(
      "contacts_phone_idx"
    ).on(table.phone),

    contactStatusIdx: index(
      "contacts_status_idx"
    ).on(table.status),

    contactTenantIdx: index(
      "contacts_tenant_idx"
    ).on(table.tenantId),

    contactStoreIdx: index(
      "contacts_store_idx"
    ).on(table.storeId),

    contactExternalIdIdx: index(
      "contacts_external_id_idx"
    ).on(table.externalId),

    contactChannelPhoneUnique: unique(
    ...
    contactStoreExternalUnique: unique(
      "contacts_store_external_unique"
    ).on(
      table.storeId,
      table.externalId
    ),
...
export const ecommerceSettings = pgTable(
  "ecommerce_settings",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    ...
    provider: varchar("provider", {
      length: 50,
    }).notNull(), // shopify, woocommerce, magento
...
export type EcommerceSettings =
  typeof ecommerceSettings.$inferSelect;

export type InsertEcommerceSettings =
  typeof ecommerceSettings.$inferInsert;
...
export const abandonedCartReminders = pgTable(
  "abandoned_cart_reminders",
  {
    ...
    templateNodeId: varchar("template_node_id", {
      length: 100,
    }).notNull(),
  ...
  (table) => ({
    cartIdx: index(
      "abandoned_cart_reminders_cart_idx"
    ).on(table.cartId),

    automationIdx: index(
      "abandoned_cart_reminders_automation_idx"
    ).on(table.automationId),

    scheduledIdx: index(
      "abandoned_cart_reminders_scheduled_idx"
    ).on(table.scheduledAt),

    statusIdx: index(
      "abandoned_cart_reminders_status_idx"
    ).on(table.status),

    uniqueReminder: unique(
      "abandoned_cart_reminders_unique"
    ).on(table.cartId, table.reminderId),
  })
);


export type AbandonedCartReminder =
  typeof abandonedCartReminders.$inferSelect;

export type InsertAbandonedCartReminder =
  typeof abandonedCartReminders.$inferInsert;
...
export const transactions = pgTable("transactions", {
  ...
  // Ziina and embedded-checkout metadata. Additive fields used by newer gateways.
...
export const paymentWebhookEvents = pgTable("payment_webhook_events", {
  ...
  sanitizedPayload: jsonb("sanitized_payload").default({}),
  processingStatus: varchar("processing_status").notNull().default("processed"),
  processedAt: timestamp("processed_at", { withTimezone: true }),
  errorMessage: text("error_message"),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow(),
}, (table) => ({
  providerPaymentIdx: index("payment_webhook_events_provider_payment_idx").on(table.provider, table.providerPaymentId),
  payloadHashUnique: uniqueIndex("payment_webhook_events_provider_hash_unique").on(table.provider, table.payloadHash),
}));
...
export const whiteLabelSettings = pgTable("white_label_settings", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  singletonKey: varchar("singleton_key", { length: 40 }).notNull().default("default").unique(),
  platformName: text("platform_name").notNull().default("Whatsway"),
  brandTagline: text("brand_tagline"),
  supportEmail: text("support_email"),
  supportPhone: text("support_phone"),
  primaryColor: varchar("primary_color", { length: 20 }).default("#16a34a"),
  secondaryColor: varchar("secondary_color", { length: 20 }).default("#111827"),
  accentColor: varchar("accent_color", { length: 20 }).default("#22c55e"),
  mainLogo: text("main_logo"),
  darkModeLogo: text("dark_mode_logo"),
  favicon: text("favicon"),
  loginBanner: text("login_banner"),
  footerText: text("footer_text"),
  customDomain: text("custom_domain"),
  emailFromName: text("email_from_name"),
  emailFromAddress: text("email_from_address"),
  hidePoweredBy: boolean("hide_powered_by").default(false),
  allowPartnerSignup: boolean("allow_partner_signup").default(false),
...
export const whiteLabelPartners = pgTable("white_label_partners", {
  ...
  revenueShareRate: numeric("revenue_share_rate", { precision: 6, scale: 2 }).default("0"),
  payoutMethod: varchar("payout_method", { length: 40 }),
  payoutDetails: jsonb("payout_details").$type<Record<string, unknown>>().default({}),
  notes: text("notes"),
...
export const whiteLabelPartnerClients = pgTable("white_label_partner_clients", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  partnerId: varchar("partner_id").notNull().references(() => whiteLabelPartners.id, { onDelete: "cascade" }),
  clientId: varchar("client_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  assignedBy: varchar("assigned_by").references((): any => users.id, { onDelete: "set null" }),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow(),
}, (table) => ({
...
export const whiteLabelWorkspaceAddons = pgTable("white_label_workspace_addons", {
  ...
  addonKey: varchar("addon_key", { length: 80 }).notNull(),
...
export const whiteLabelPlanConfigs = pgTable("white_label_plan_configs", {
  ...
  billingCycle: varchar("billing_cycle", { length: 20 }).default("half_yearly"),
...
export const whiteLabelCreditTransactions = pgTable("white_label_credit_transactions", {
  ...
  credits: numeric("credits", { precision: 14, scale: 2 }).notNull(),
  balanceBefore: numeric("balance_before", { precision: 14, scale: 2 }).notNull().default("0"),
...
export const whiteLabelAuditLogs = pgTable("white_label_audit_logs", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  actorId: varchar("actor_id").references((): any => users.id, { onDelete: "set null" }),
  actionType: varchar("action_type", { length: 80 }).notNull(),
  targetType: varchar("target_type", { length: 80 }).notNull(),
  targetId: varchar("target_id"),
  previousValues: jsonb("previous_values").$type<Record<string, unknown>>(),
  updatedValues: jsonb("updated_values").$type<Record<string, unknown>>(),
