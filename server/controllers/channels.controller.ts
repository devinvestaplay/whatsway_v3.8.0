...
export const createChannel = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const url = `https://graph.facebook.com/${WHATSAPP_API_VERSION}/${channel.phoneNumberId}?fields=${fields}`;
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      const deregisterRes = await fetch(
        `https://graph.facebook.com/${WHATSAPP_API_VERSION}/${channel.phoneNumberId}/deregister`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${channel.accessToken}`,
...
export const embeddedSignup = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const debugRes = await fetch(
      `https://graph.facebook.com/${WHATSAPP_API_VERSION}/debug_token?input_token=${longLivedToken}&access_token=${appId}|${appSecret}`
    );
    ...
    if (!wabaId) {
      ...
      const businessRes = await fetch(
        `https://graph.facebook.com/${WHATSAPP_API_VERSION}/me?fields=businesses&access_token=${longLivedToken}`
      );
      ...
      if (businessId) {
        const wabaRes = await fetch(
          `https://graph.facebook.com/${WHATSAPP_API_VERSION}/${businessId}/owned_whatsapp_business_accounts?access_token=${longLivedToken}`
        );
    ...
    const phoneRes = await fetch(
      `https://graph.facebook.com/${WHATSAPP_API_VERSION}/${wabaId}/phone_numbers?access_token=${longLivedToken}`
    );
...
export const updateDisplayName = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const url = `https://graph.facebook.com/${WHATSAPP_API_VERSION}/${channel.phoneNumberId}`;
    const response = await fetch(url, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${channel.accessToken}`,
...
export const getBusinessProfile = asyncHandler(async (req: Request, res: Response) => {
  ...
  const url = `https://graph.facebook.com/${WHATSAPP_API_VERSION}/${channel.phoneNumberId}/whatsapp_business_profile?fields=about,address,description,email,profile_picture_url,websites,vertical`;
  const response = await fetch(url, {
    headers: { Authorization: `Bearer ${channel.accessToken}` },
  });
...
export const updateBusinessProfile = asyncHandler(async (req: Request, res: Response) => {
  ...
  const url = `https://graph.facebook.com/${WHATSAPP_API_VERSION}/${channel.phoneNumberId}/whatsapp_business_profile`;
...
export const testCredentials = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const tokenRes = await fetch(
      `https://graph.facebook.com/${WHATSAPP_API_VERSION}/oauth/access_token?client_id=${config.appId}&client_secret=${config.appSecret}&grant_type=client_credentials`
    );
    const tokenData: any = await tokenRes.json();
    ...
    if (tokenData.access_token) {
      ...
      try {
        const appRes = await fetch(
          `https://graph.facebook.com/${WHATSAPP_API_VERSION}/${config.appId}?fields=name&access_token=${tokenData.access_token}`
        );
