...
app.get("/.well-known/apple-developer-merchantid-domain-association", (_req, res) => {
  ...
  if (!fs.existsSync(filePath)) {
    return res.status(404).type("text/plain").send("Ziina Apple merchant domain verification file is not installed.");
  }
...
app.use('/api/webhooks/ziina', express.raw({ type: 'application/json' }));
app.use(
  express.json({
    limit: '50mb',
