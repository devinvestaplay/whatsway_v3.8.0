...
function HeroVisual() {
  return (
    <div className="relative mx-auto max-w-xl">
      <div className="absolute -left-5 top-12 hidden rounded-full bg-slate-50 px-4 py-3 text-xs font-black text-slate-700 shadow-lg ring-1 ring-slate-100 md:block">Reach 1 Lakh package</div>
      <div className="absolute -right-4 top-28 hidden rounded-full bg-white px-4 py-3 text-xs font-black text-slate-700 shadow-lg ring-1 ring-slate-100 md:block">INR 4,00,000 generated</div>
      <div className="rounded-[2rem] bg-slate-50 p-5 shadow-2xl shadow-slate-950/10 ring-1 ring-slate-100">
        <div className="rounded-[1.5rem] bg-white p-5 shadow-lg">
          <div className="flex items-center justify-between">
            <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-700">Campaign Sent</span>
            <span className="text-xs font-black text-slate-600">100k clicks</span>
          </div>
          <div className="mt-5 rounded-2xl bg-slate-50 p-4">
            <div className="aspect-[4/2.4] rounded-xl bg-gradient-to-br from-slate-100 via-white to-slate-200 ring-1 ring-slate-100" />
          ...
          <div className="mt-5 grid grid-cols-3 gap-3">
            {[["98%", "Open"], ["52%", "Click"], ["28%", "Reply"]].map(([value, label]) => (
              <div key={label} className="rounded-xl bg-slate-50 p-3 text-center ring-1 ring-slate-100">
                <p className="text-lg font-black text-slate-900">{value}</p>
                <p className="text-xs text-slate-500">{label}</p>
              </div>
            ))}
          </div>
          <button className="mt-5 h-10 w-full rounded-md bg-slate-950 text-sm font-black text-white">Send Payment Link</button>
        </div>
      </div>
    </div>
  );
...
export default function ProductWhatsappMarketingPage() {
  ...
  const { data: cmsCaseStudies } = useQuery<{ rows: CmsEntry[] }>({
    queryKey: ["/api/cms/entries", "case_study"],
    queryFn: () => fetch("/api/cms/entries?type=case_study").then((res) => res.json()),
    staleTime: 5 * 60 * 1000,
  });
  const { data: cmsReviews } = useQuery<{ rows: CmsEntry[] }>({
    queryKey: ["/api/cms/entries", "review_feedback"],
    queryFn: () => fetch("/api/cms/entries?type=review_feedback").then((res) => res.json()),
    staleTime: 5 * 60 * 1000,
  });
  ...
  const displayFounderLogos = cmsLogoResponse?.rows?.length
    ...
    : founderLogos;
  const displayCases = cmsCaseStudies?.rows?.length
    ? cmsCaseStudies.rows.map((entry) => [
        entry.title,
        entry.subtitle || entry.metadata?.metric || "Case study",
        entry.body || entry.metadata?.metric || "See how this customer used WhatsApp automation to improve growth.",
      ])
    : cases;
  const displayTestimonials = cmsReviews?.rows?.length
    ? cmsReviews.rows.map((entry) => [
        entry.title,
        entry.subtitle || "Customer",
        entry.body || entry.metadata?.metric || "Great experience with the platform.",
      ])
    : testimonials;
    ...
    <div className="bg-white text-slate-950">
      <section className="bg-white pb-12 pt-10">
        <div className="mx-auto grid max-w-[1180px] gap-10 px-4 sm:px-6 lg:grid-cols-[0.92fr_1.08fr] lg:items-center lg:px-8">
          <div className="text-center lg:text-left">
            <p className="text-sm font-black text-slate-600">WhatsApp Marketing Software</p>
            <h1 className="mt-3 text-4xl font-black leading-tight tracking-normal sm:text-6xl">
              Meet India's Best <span className="block text-slate-700">WhatsApp Marketing Software</span>
            </h1>
            ...
            <div className="mt-6 flex flex-col justify-center gap-3 sm:flex-row lg:justify-start">
              <Link href="/signup" className="inline-flex h-12 items-center justify-center rounded-md bg-slate-950 px-6 text-sm font-black text-white transition hover:bg-slate-800">
      ...
      <section className="py-10">
        <div className="mx-auto grid max-w-[1180px] gap-8 px-4 sm:px-6 lg:grid-cols-[0.95fr_1.05fr] lg:items-center lg:px-8">
          <div>
            ...
            <Link href="/signup" className="mt-5 inline-flex rounded-md bg-slate-950 px-5 py-3 text-sm font-black text-white">Learn More</Link>
          </div>
      ...
      <section className="bg-white py-12">
        <div className="mx-auto max-w-[1180px] px-4 sm:px-6 lg:px-8">
          <div className="rounded-[2rem] border border-slate-200 bg-white p-7 shadow-2xl shadow-slate-950/8 sm:p-10 lg:p-12">
            <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
              <div>
                <p className="text-sm font-black uppercase tracking-[0.24em] text-slate-600">Why WhatsApp?</p>
                <h2 className="mt-6 text-4xl font-black leading-tight text-slate-950 sm:text-5xl lg:text-6xl">
                  WhatsApp brings notifications, marketing, and support into one customer channel.
                </h2>
                <p className="mt-5 max-w-2xl text-sm leading-7 text-slate-600">
                  Reach customers where they already respond, then automate campaigns, replies, and sales follow-ups from one workspace.
                </p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                {stats.map((stat) => (
                  <div key={stat.label} className="flex min-h-[140px] flex-col items-center justify-center rounded-[1.5rem] bg-slate-50 p-5 text-center ring-1 ring-slate-100">
                    <p className="text-4xl font-black leading-none text-slate-950">{stat.value}</p>
      ...
      <section className="py-12">
        <div className="mx-auto max-w-[1180px] px-4 text-center sm:px-6 lg:px-8">
          ...
          <div className="relative mx-auto mt-8 max-w-3xl overflow-hidden rounded-xl bg-slate-200 shadow-2xl shadow-slate-950/15">
            <div className="aspect-video bg-gradient-to-br from-slate-50 via-white to-slate-200 p-8">
              <div className="flex h-full items-center justify-center">
                <div className="rounded-xl bg-white/85 p-8 text-center shadow-xl">
                  <p className="text-4xl font-black text-slate-900">{appName} Explained</p>
                  <p className="mt-2 text-xl font-black text-slate-950">in 3 Minutes</p>
                  <div className="mx-auto mt-6 flex h-16 w-16 items-center justify-center rounded-full bg-slate-950 text-white">
        ...
        <div className="mx-auto max-w-[1180px] px-4 text-center sm:px-6 lg:px-8">
          ...
          <div className="mt-7 flex flex-wrap justify-center gap-2">
            {industryTabs.map((tab, index) => (
              <button key={tab} className={`rounded-full px-5 py-2 text-sm font-black ${index === 0 ? "bg-slate-950 text-white" : "bg-slate-100 text-slate-700"}`}>{tab}</button>
            ))}
          </div>
          <div className="mx-auto mt-8 grid max-w-4xl gap-6 rounded-[1.5rem] bg-slate-50 p-5 text-left md:grid-cols-[0.8fr_1.2fr]">
            <div className="rounded-xl bg-gradient-to-br from-slate-100 via-white to-slate-200 ring-1 ring-slate-100" />
            <div className="rounded-xl bg-white p-6 shadow-lg">
              ...
              <ul className="mt-5 space-y-3">
                {["Broadcast promotional offers", "Send abandoned cart reminders", "Automate shipping and payment alerts"].map((item) => (
                  <li key={item} className="flex gap-3 text-sm font-semibold text-slate-700"><CheckCircle2 className="h-5 w-5 text-slate-600" />{item}</li>
                ))}
              </ul>
            </div>
          </div>
      ...
      <section className="bg-slate-50 py-12">
        <div className="mx-auto max-w-[900px] px-4 sm:px-6 lg:px-8">
          <div className="grid gap-4 rounded-xl bg-white p-6 shadow-xl shadow-slate-950/5 ring-1 ring-slate-100 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <section className="py-12">
        <div className="mx-auto max-w-[1180px] px-4 text-center sm:px-6 lg:px-8">
          ...
          <div className="mt-8 grid gap-5 md:grid-cols-3">
            {displayCases.map(([title, role, text]) => (
      ...
      <section className="bg-slate-50 py-12">
        <div className="mx-auto max-w-[1180px] px-4 text-center sm:px-6 lg:px-8">
          <div className="grid gap-5 md:grid-cols-3">
            {displayTestimonials.map(([name, role, quote]) => (
              <div key={name} className="rounded-xl bg-white p-6 shadow-lg shadow-slate-950/5">
                <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-slate-100 text-xl font-black text-slate-700">{name[0]}</div>
      ...
      <section className="py-8">
        <div className="mx-auto flex max-w-[1180px] flex-col items-center justify-between gap-4 rounded-xl border border-slate-100 bg-white px-5 py-5 shadow-lg shadow-slate-950/5 sm:px-6 md:flex-row lg:px-8">
          ...
          <div className="flex gap-3">
            ...
            <Link href="/signup" className="rounded-md bg-slate-950 px-5 py-3 text-sm font-black text-white">Start FREE</Link>
          </div>
        </div>
      </section>
